...
static void init_thread( struct thread *t, const char *name, int priority ) {
    ...
    t->stack_allocated_boundary = NULL;
    t->stack_rsp = NULL;
}
...
static void do_schedule( int status ) {
    ...
    while ( !list_empty( &destruction_req ) ) {                                                         // 파괴 요청 목록이 비어 있지 않은 경우
        struct thread *victim = list_entry( list_pop_front( &destruction_req ), struct thread, elem );  // 파괴할 스레드를 찾음
