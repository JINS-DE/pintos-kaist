...
void check_file_handle( int fd, const char *file_name, const void *buf_, size_t size ) {
    ...
    if ( file_size != size ) msg( "size of %s (%zu) differs from expected (%zu)", file_name, file_size, size );
    ...
    while ( ofs < size ) {
        ...
        if ( ret_val != block_size ) fail( "read of %zu bytes at offset %zu in \"%s\" returned %zu", block_size, ofs, file_name, ret_val );
    ...
    if ( file_size != size ) fail( "size of %s (%zu) differs from expected (%zu)", file_name, file_size, size );
