...
static bool make_tar_archive( const char *archive_name, char *files[], size_t file_cnt ) {
    ...
    for ( i = 0; i < file_cnt; i++ ) {
        ...
        if ( !archive_file( file_name, sizeof file_name, archive_fd, &write_error ) ) success = false;
    }

    if ( !do_write( archive_fd, zeros, 512, &write_error ) || !do_write( archive_fd, zeros, 512, &write_error ) ) success = false;
...
static bool archive_ordinary_file( const char *file_name, int file_fd, int archive_fd, bool *write_error ) {
    ...
    if ( !write_header( file_name, '0', file_size, 0644, archive_fd, write_error ) ) return false;
...
static bool archive_directory( char file_name[], size_t file_name_size, int file_fd, int archive_fd, bool *write_error ) {
    ...
    if ( !write_header( file_name, '5', 0, 0755, archive_fd, write_error ) ) return false;

    file_name[dir_len] = '/';
    while ( readdir( file_fd, &file_name[dir_len + 1] ) )
        if ( !archive_file( file_name, file_name_size, archive_fd, write_error ) ) success = false;
    file_name[dir_len] = '\0';
...
static bool write_header( const char *file_name, char type_flag, int size, unsigned mode, int archive_fd, bool *write_error ) {
    ...
    /* Drop confusing and possibly dangerous prefixes from
       FILE_NAME. */
    while ( *file_name == '/' || !memcmp( file_name, "./", 2 ) || !memcmp( file_name, "../", 3 ) ) file_name = strchr( file_name, '/' ) + 1;
    if ( *file_name == '\0' ) {
