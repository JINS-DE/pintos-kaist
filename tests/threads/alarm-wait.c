...
static void test_sleep( int thread_cnt, int iterations ) {
    ...
    if ( threads == NULL || output == NULL ) PANIC( "couldn't allocate memory for test" );
    ...
    for ( i = 0; i < thread_cnt; i++ )
        if ( threads[i].iterations != iterations ) fail( "thread %d woke up %d times instead of %d", i, threads[i].iterations, iterations );
